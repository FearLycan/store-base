...
final class StoreController extends Controller
{
    ...
    public function actionView(string $slug): string
    {
        ...
        // The category filter arrives as a slug (pretty /store/<slug>/<category>
        // URLs, the category tiles) or a numeric id (the filter select). Resolve
        // it once — for breadcrumbs, the page title and the canonical URL.
        $activeCategory = null;
        if (!empty($filters['category'])) {
            $activeCategory = is_numeric($filters['category'])
                ? Category::findOne((int)$filters['category'])
                : Category::findOne(['slug' => (string)$filters['category']]);
        }
