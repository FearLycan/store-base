<?php
/**
 * Store social / web links: a row of branded icon buttons, rendered only for the
 * links the store actually has. Shared so the presentation lives in one place.
...
<div class="mt-3 flex flex-wrap items-center gap-2">
    <?php foreach ($links as $l): ?>
        <a href="<?= Html::encode($l['url']) ?>" target="_blank" rel="nofollow noopener"
           class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition-colors hover:border-[color:var(--accent)] hover:text-[color:var(--accent)]"
           aria-label="<?= Html::encode($l['label']) ?>" title="<?= Html::encode($l['label']) ?>">
            <?= $icons[$l['icon']] ?>
        </a>
    <?php endforeach; ?>
</div>
